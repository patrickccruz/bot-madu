...
module.exports = {
  ...
  description: " Falar ai irmão", // Coloque a descrição do comando
  ...
  run: async (client, interaction) => {
    ...
    if (!interaction.member.permissions.has(Discord.PermissionFlagsBits.ManageMessages)) {
        ...
        if (!embed_fala && !normal_fala) {
            interaction.reply(`Escreva pelo menos em uma das opções ai irmão!!.`)
        } else {
            ...
            if (embed_fala === "⠀") {
                interaction.reply({ content: ` ja mandei la irmão se cuida..`, ephemeral: true })
